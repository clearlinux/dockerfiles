...
EOF
    ...
    if [[ -n "${postgresql_data:-}" ]]; then
        if [[ -d "${postgresql_data}/data" ]] || [[ "${postgresql_data}" = "/clrconf/postgresql" ]]; then
...
EOF
    ...
    cat <<"EOF"
# Paths
export POSTGRESQL_VOLUME_DIR="${POSTGRESQL_VOLUME_DIR:-/clrconf/postgresql}"
export POSTGRESQL_DATA_DIR="${POSTGRESQL_DATA_DIR:-$POSTGRESQL_VOLUME_DIR/data}"
export POSTGRESQL_BASE_DIR="/var/lib/pgsql/"
export POSTGRESQL_CONF_DIR="/etc/postgresql11"
export POSTGRESQL_MOUNTED_CONF_DIR="/clrconf/postgresql/conf"
...
export POSTGRESQL_BIN_DIR="/usr/libexec/postgresql11"
...
export POSTGRESQL_RUNTIME_DIR="/run/postgresql11"
export POSTGRESQL_INSTALL_DATA="/usr/share/postgresql11"
...
postgresql_create_config() {
    ...
    cp "$POSTGRESQL_INSTALL_DATA/postgresql.conf.sample" "$POSTGRESQL_CONF_FILE"
...
postgresql_initialize() {
    ...
    ensure_dir_exists "$POSTGRESQL_RUNTIME_DIR"
    am_i_root && chown "$POSTGRESQL_DAEMON_USER:$POSTGRESQL_DAEMON_GROUP" "$POSTGRESQL_RUNTIME_DIR"
    #Need to do : Do we need to set 775 here?
    am_i_root && chmod 775 "$POSTGRESQL_RUNTIME_DIR"
	...
	postgresql_info "$POSTGRESQL_DATA_DIR is empty"
        ensure_dir_exists "$POSTGRESQL_DATA_DIR"
        am_i_root && chown "$POSTGRESQL_DAEMON_USER:$POSTGRESQL_DAEMON_GROUP" "$POSTGRESQL_DATA_DIR"
	am_i_root && chmod -R 775 "$POSTGRESQL_DATA_DIR"
        if [[ "$POSTGRESQL_REPLICATION_MODE" = "master" ]]; then
	    postgresql_info "$POSTGRESQL_POSTGRES_PASSWORD"
	    postgresql_info "$POSTGRESQL_USERNAME"
...
postgresql_execute() {
    ...
    [[ -n "$db" ]] && args+=( "-d" "$db" )
    [[ -n "$opts" ]] && args+=( "$opts" )
    if [[ "${CLEAR_DEBUG:-false}" = true ]]; then
...
postgresql_start_bg() {
    ...
    su-exec "$POSTGRESQL_DAEMON_USER" "$POSTGRESQL_BIN_DIR"/pg_ctl "start" "${pg_ctl_flags[@]}"
...
postgresql_master_init_db() {
    ...
    if [[ -n "${initdb_args[*]:-}" ]]; then
        ...
        if [[ "${CLEAR_DEBUG:-false}" = true ]]; then
            su-exec "$POSTGRESQL_DAEMON_USER" "$POSTGRESQL_BIN_DIR/initdb" -E UTF8 -D "$POSTGRESQL_DATA_DIR" -U "postgres" "${initdb_args[@]}"
        else
            su-exec "$POSTGRESQL_DAEMON_USER" "$POSTGRESQL_BIN_DIR/initdb" -E UTF8 -D "$POSTGRESQL_DATA_DIR" -U "postgres" "${initdb_args[@]}" >/dev/null 2>&1
        fi
    elif [[ "${CLEAR_DEBUG:-false}" = true ]]; then
        su-exec "$POSTGRESQL_DAEMON_USER" "$POSTGRESQL_BIN_DIR/initdb" -E UTF8 -D "$POSTGRESQL_DATA_DIR" -U "postgres"
    else
        su-exec "$POSTGRESQL_DAEMON_USER" initdb -d -E UTF8 -D "$POSTGRESQL_DATA_DIR" -U "postgres" >/dev/null 2>&1
    fi
}
...
postgresql_slave_init_db() {
    ...
    local -r backup_cmd=(su-exec "$POSTGRESQL_DAEMON_USER" "$POSTGRESQL_BIN_DIR"/pg_basebackup)
...
postgresql_configure_recovery() {
    ...
    if (( POSTGRESQL_MAJOR_VERSION >= 12 )); then
        ...
        cp -f "$POSTGRESQL_INSTALL_DATA/recovery.conf.sample" "$POSTGRESQL_RECOVERY_FILE"
        chmod 666 "$POSTGRESQL_RECOVERY_FILE"
